    ...
    for u in unidades:
        ...
        for t in (u.get("temas") or []):
            ...
            idx.append({
                ...
                "palabras_clave": t.get("palabras_clave") or [],
            })
...
def _score(qn: str, it: Dict[str, Any]) -> int:
    ...
    # Boost score if "mvc" or "modelo vista controlador" is in both the query and the topic name
    if ("mvc" in qn or "modelo vista controlador" in qn or "modelo-vista-controlador" in qn) and "mvc" in tk:
        s += 60

    # Match against palabras_clave
    for pk in it.get("palabras_clave", []):
        pk_norm = _normalize(pk)
        if pk_norm and pk_norm in qn:
            s += 50

    # Penalize topics that are not about MVC if the query is specifically about MVC
    if ("mvc" in qn or "modelo vista controlador" in qn or "modelo-vista-controlador" in qn) and "mvc" not in tk:
        s -= 100
...
def responder_local_temas(
    ...
    # Force selection of "Patrón MVC" if the query contains "mvc" or its variations
    if "mvc" in qn or "modelo vista controlador" in qn or "modelo-vista-controlador" in qn:
        for it in idx:
            if "mvc" in it.get("k", ""):
                best_it = it
                best_sc = 100
    ...
    if not best_it:
        for it in idx:
            sc = _score(qn, it)
            # Bypass level filter for direct strong matches (score >= 60)
            if sc < 60 and not _nivel_ok(nivel, it.get("nivel", "")):
                continue
            if sc > best_sc:
                best_sc = sc
                best_it = it
