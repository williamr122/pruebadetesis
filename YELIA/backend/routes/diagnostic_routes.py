    ...
    try:
        ...
        if done:
            try:
                ...
                from backend.services.progreso_service import cargar_progreso
                prog = cargar_progreso(usuario) or {}
                profile_data = {
                    "alias": alias,
                    "ciclo": prog.get("ciclo_academico") or prof.get("course") or "",
                    "estado": prog.get("estado_materia") or "",
                    "nivel": prog.get("nivel_materia") or prof.get("level_current") or "Basico"
                }
            except Exception as e:
                logger.warning("Error al cargar perfil en diagnostic status", error=str(e))
                profile_data = {
                    "alias": alias,
                    "ciclo": "",
                    "estado": "",
        ...
        return _ok({"usuario": usuario, "alias": alias, "mode": mode, "diagnostic_completed": done, "profile": profile_data})
