...
def api_students_list():
    ...
    if request.method == "POST":
        ...
        # Force STU- prefix for student accounts in db to unify with login/chat logic
        db_alias = alias if alias.startswith("STU-") else f"STU-{alias}"
        ...
        _audit("student.create", target=db_alias, meta={"email": email})
        return _ok({"id": new_id, "alias": db_alias}, 201)
...
def api_student_activity(alias: str):
    ...
    # Evitar doble prefijo si ya viene con STU-
    if alias.startswith("STU-"):
        stu_usuario = alias
    else:
...
def api_student_profile_get(alias: str):
    ...
    # Normalize student alias to use STU- prefix
    if not alias.startswith("STU-") and not alias.startswith("GUEST-") and not alias.startswith("Anon-"):
        alias = f"STU-{alias}"
...
def api_student_profile_patch(alias: str):
    ...
    # Sync academic profile (level and cycle) to progreso table
    ciclo = merged.get("course")
    nivel = merged.get("level_current")
    nivel_formatted = nivel.capitalize() if nivel else None
    ...
    try:
        ...
        actualizar_perfil_usuario(
            ...
            nivel_materia=nivel_formatted
        )
    except Exception as exc:
        logger.warning("No se pudo sincronizar perfil academico con progreso", error=str(exc))
