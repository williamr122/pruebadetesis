...
def _resource_recommendations(
    *,
    ...
    if not force:
        return []
    local_resources = resources_for_topic(topic_label, limit=10)
    type_labels = {
        "pdf": "📄 PDF",
    ...
    local_recs = [
        _make_rec(
            ...
            label=type_labels.get(resource.get("type"), "Recurso oficial"),
            title=resource.get("title") or "Recurso de la unidad",
            explanation=resource.get("description") or f"Material base de {resource.get('unit_title') or 'la ruta academica'} registrado en el ZIP academico.",
            ...
            url=resource.get("source") if resource.get("source") and resource.get("source").startswith("/resources") else None,
            extra={
