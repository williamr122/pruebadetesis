...
# Let's find eye vertices (ojo offset > 0.01)
eye_vertices = []
...
print(f"Found {len(eye_vertices)} eye vertices.")
if len(eye_vertices) > 0:
    eye_z = [v[3] for v in eye_vertices]
    print(f"Z Range of these eye vertices: Min={min(eye_z):.4f}, Max={max(eye_z):.4f}")
