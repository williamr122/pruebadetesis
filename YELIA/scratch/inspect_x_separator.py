...
# Let's find eye vertices (ojo offset > 0.01) that have absY < 0.08
eye_vertices = []
for i in range(len(base_positions)):
    x, y, z = base_positions[i]
    if np.linalg.norm(ojo_offsets[i]) > 0.01 and abs(y) < 0.08:
        eye_vertices.append((i, x, y, z))

print(f"Found {len(eye_vertices)} eye vertices with abs(Y) < 0.08.")
if len(eye_vertices) > 0:
    eye_x = [v[1] for v in eye_vertices]
    print(f"X Range of these eye vertices: Min={min(eye_x):.4f}, Max={max(eye_x):.4f}")
    
# Let's check beak vertices (boca offset > 0.005)
beak_vertices = []
for i in range(len(base_positions)):
    x, y, z = base_positions[i]
    if np.linalg.norm(boca_offsets[i]) > 0.005:
        beak_vertices.append((i, x, y, z))

print(f"\nFound {len(beak_vertices)} beak vertices total.")
if len(beak_vertices) > 0:
    beak_x = [v[1] for v in beak_vertices]
    print(f"X Range of these beak vertices: Min={min(beak_x):.4f}, Max={max(beak_x):.4f}")
