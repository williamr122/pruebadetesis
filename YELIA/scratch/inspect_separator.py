...
# Beak vertices: boca offset > 0.005, X > 0.40
# Eye vertices: ojo offset > 0.01
beak_abs_y = []
eye_abs_y = []

for i in range(len(base_positions)):
    x, y, z = base_positions[i]
    # Check if affected by boca (beak)
    if x > 0.40 and np.linalg.norm(boca_offsets[i]) > 0.005:
        beak_abs_y.append(abs(y))
    # Check if affected by ojo (eye)
    if np.linalg.norm(ojo_offsets[i]) > 0.01:
        eye_abs_y.append(abs(y))

beak_abs_y = np.array(beak_abs_y)
eye_abs_y = np.array(eye_abs_y)

print(f"Beak abs(Y) range: Min={np.min(beak_abs_y):.4f}, Max={np.max(beak_abs_y):.4f}")
print(f"Eye abs(Y) range: Min={np.min(eye_abs_y):.4f}, Max={np.max(eye_abs_y):.4f}")

# Check factors for both under proposed logic
def get_factor(absY):
    if absY >= 0.05:
        return 0.0
    elif absY > 0.047:
        return (0.05 - absY) / 0.003
    return 1.0

beak_factors = [get_factor(y) for y in beak_abs_y]
eye_factors = [get_factor(y) for y in eye_abs_y]

print(f"\nProposed logic verification:")
