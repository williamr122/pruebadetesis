...
function codeBlock() {
  return (
    <pre className="yelia-code-block">
      <code className="language-java">System.out.println("Hola Soy YELIA");</code>
    </pre>
  );
}
...
export default function ChatApp() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      const activeStudent = readStorage('yelia_active_student', '');
      const aliasToUse = localProfile.alias || activeStudent;

      try {
        let who = await api.get('/api/auth/whoami');

        // Auto-login if there's an active student but the backend session is guest/anonymous/mismatched
        const isGuestSession = !who?.usuario || who?.mode === 'guest' || String(who?.usuario || '').startsWith('GUEST-') || String(who?.usuario || '').startsWith('Anon-');
        const expectedUser = aliasToUse ? `STU-${aliasToUse.replace(/[^A-Za-z0-9._ -]/g, '').replace(/\s+/g, '-')}` : '';

        if (aliasToUse && (isGuestSession || (who?.usuario && who?.usuario !== expectedUser))) {
          try {
            const loginRes = await api.post('/api/auth/login', { student_code: aliasToUse });
            if (loginRes.usuario) {
              who = await api.get('/api/auth/whoami');
            }
          } catch (loginErr) {
            console.error("Auto login failed on boot", loginErr);
          }
        }

        if (!alive) return;
        const isGuest = !who?.usuario || who?.mode === 'guest' || String(who?.usuario || '').startsWith('GUEST-') || String(who?.usuario || '').startsWith('Anon-');
        let visibleName = localProfile.alias || who?.nickname || (isGuest ? 'Invitado 1' : who?.usuario) || 'Invitado 1';
        if (typeof visibleName === 'string' && visibleName.startsWith('STU-')) {
          visibleName = visibleName.slice(4);
        }
        ...
        const rawAlias = localProfile.alias || '';
        const cleanAliasName = rawAlias.startsWith('STU-') ? rawAlias.slice(4) : rawAlias;
        setUser(localGuest ? 'Invitado 1' : (cleanAliasName || 'Invitado 1'));
      }
      try {
