...
export default function RutaClient() {
  ...
  useEffect(() => {
    let alive = true;
    api.get('/api/progreso')
      .then((data) => {
        if (!alive) return;
        if (data?.progreso) {
          const local = readProfile();
          const next = {
            alias: local.alias || localStorage.getItem('yelia_active_student') || data.progreso.usuario?.replace('STU-', '') || 'Invitado 1',
            ciclo: data.progreso.ciclo_academico || local.ciclo || '',
            estado: data.progreso.estado_materia || local.estado || '',
            nivel: data.progreso.nivel_materia || local.nivel || 'Basico',
          };
