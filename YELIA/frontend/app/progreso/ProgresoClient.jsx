    ...
    Promise.allSettled([api.get('/api/progreso'), api.get('/api/learning-route')])
      .then((results) => {
        ...
        if (progressResult.status === 'fulfilled') {
          const prog = progressResult.value.progreso || {};
          setProgress(prog);
          ...
          const next = {
            alias: local.alias || localStorage.getItem('yelia_active_student') || prog.usuario?.replace('STU-', '') || 'Invitado 1',
            ciclo: prog.ciclo_academico || local.ciclo || '',
            estado: prog.estado_materia || local.estado || '',
            nivel: prog.nivel_materia || local.nivel || 'Basico',
          };
