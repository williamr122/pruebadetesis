...
export default function ProgresoClient() {
  ...
  useEffect(() => {
    ...
    async function syncAndLoad() {
      try {
        const activeAlias = localStorage.getItem('yelia_active_student') || '';
        const localProfile = readProfile();
        let effectiveGuest = localStorage.getItem('yelia_guest_mode') === '1' || !activeAlias;
        
        if (activeAlias && !effectiveGuest) {
          await api.post('/api/auth/login', { student_code: activeAlias }).catch(() => {});
        }
        
        const who = await api.get('/api/auth/whoami').catch(() => null);
        const progressData = await api.get('/api/progreso').catch(() => null);
        const routeResult = await api.get('/api/learning-route').catch(() => null);
        ...
        if (progressData?.progreso) {
          ...
          const updatedProfile = {
            ...localProfile,
            alias: who?.nickname || activeAlias || prog.usuario?.replace(/^STU-/, '') || 'Estudiante',
            ciclo: prog.ciclo_academico || localProfile.ciclo || '',
            estado: prog.estado_materia || localProfile.estado || '',
            nivel: prog.nivel_materia || localProfile.nivel || 'Por diagnosticar',
          };
          setProfile(updatedProfile);
          localStorage.setItem('yelia_profile', JSON.stringify(updatedProfile));
          if (activeAlias) {
            const studentId = `STU-${activeAlias.replace(/[^A-Za-z0-9._ -]/g, '').trim().replace(/\s+/g, '-')}`;
            localStorage.setItem(`yelia_profile_${studentId}`, JSON.stringify(updatedProfile));
          }
        ...
        if (routeResult?.route) {
          setRouteState(routeResult.route);
        }
      } catch (err) {
        if (!alive) return;
  ...
  const unitRows = useMemo(() => UNITS.map((unit) => {
    const value = progressFor(unit, routeState);
