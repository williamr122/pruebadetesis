...
function saveStudentProfile(alias: string) {
  ...
  if (alias) {
    const studentId = `STU-${alias.replace(/[^A-Za-z0-9._ -]/g, '').trim().replace(/\s+/g, '-')}`;
    localStorage.setItem('yelia_active_student', alias);
    localStorage.setItem('yelia_active_student_id', studentId);
    localStorage.setItem(`yelia_profile_${studentId}`, JSON.stringify(next));
  }
}
