...
function saveProfile(profile, mode) {
  ...
  if (mode === 'guest') {
    ...
    localStorage.removeItem('yelia_active_student_id');
  } else {
    const studentId = `STU-${String(profile.alias || '').replace(/[^A-Za-z0-9._ -]/g, '').trim().replace(/\s+/g, '-')}`;
    localStorage.setItem('yelia_active_student', profile.alias);
    localStorage.setItem('yelia_active_student_id', studentId);
    localStorage.setItem(`yelia_profile_${studentId}`, JSON.stringify(clean));
  }
}
...
export default function DiagnosticClient() {
  ...
  async function loadQuestions() {
    ...
    try {
      ...
      const finalAlias = status.alias || cleanAlias;
      const studentId = status.usuario || `STU-${cleanAlias.replace(/[^A-Za-z0-9._ -]/g, '').trim().replace(/\s+/g, '-')}`;

      if (status.diagnostic_completed) {
        localStorage.setItem('yelia_active_student', finalAlias);
        localStorage.setItem('yelia_active_student_id', studentId);
        
        const cleanProfile = {
          alias: finalAlias,
          ciclo: status.ciclo || cycle,
          estado: status.estado || state,
          nivel: status.nivel || 'Por diagnosticar',
          diagnostic: status.diagnostic || null,
        };
        localStorage.setItem('yelia_profile', JSON.stringify(cleanProfile));
        localStorage.setItem(`yelia_profile_${studentId}`, JSON.stringify(cleanProfile));
        localStorage.setItem('yelia_guest_mode', mode === 'guest' ? '1' : '0');
      ...
      let nextState = state;
      if (status.ciclo) {
        setCycle(status.ciclo);
        nextCycle = status.ciclo;
      }
      if (status.estado) {
        setState(status.estado);
        nextState = status.estado;
      }
          ...
          {mode !== 'guest' && (
            <>
              ...
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', width: '100%' }}>
                ...
                >
                  <i className={`bi ${showPwd ? 'bi-eye-slash-fill' : 'bi-eye-fill'}`} style={{ fontSize: '1.2rem' }}></i>
                </span>
                <input
                  id="diagPassword"
                  className="diag-input"
