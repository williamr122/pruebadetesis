                ...
                {activeStudents.length ? activeStudents.map((student) => (
                  <button className="teacher-live-row" type="button" key={student.id || studentLabel(student)} onClick={() => { setSelectedStudent(student); go('students'); }}>
                    ...
                    <span className="teacher-live-main">
                      ...
                      <small>{student.role || 'student'} - {formatStatus(student.status || 'active')}</small>
                    </span>
                    <span className="teacher-live-time">{fmtDate(student.last_seen || student.updated_at || student.created_at)}</span>
                    <i className="teacher-live-dot" style={student.status === 'blocked' || student.status === 'inactive' ? { background: '#ef4444', boxShadow: '0 0 0 5px rgba(239, 68, 68, .14)' } : {}}></i>
                  </button>
        ...
        {view === 'students' && <section id="students" className="teacher-card">
          ...
          {showStudentForm ? <div className="teacher-create-student-card">
            <div>
              ...
              <p>Agrega un estudiante al seguimiento y configura su ciclo y estado inicial respecto a la materia.</p>
            </div>
            ...
            <select value={newStudentForm.status} onChange={(e) => setNewStudentForm({ ...newStudentForm, status: e.target.value })}><option value="active">Activo</option><option value="blocked">Desconectado</option></select>
            
            <select value={newStudentForm.course} onChange={(e) => setNewStudentForm({ ...newStudentForm, course: e.target.value })}>
            ...
            <select value={newStudentForm.tags} onChange={(e) => setNewStudentForm({ ...newStudentForm, tags: e.target.value })}>
          ...
          {editingStudent ? <div className="teacher-edit-card">
            ...
            <select value={editingStudent.status || 'active'} onChange={(e) => setEditingStudent({ ...editingStudent, status: e.target.value })}><option value="active">Activo</option><option value="blocked">Desconectado</option></select>
          ...
          {selectedStudent ? <div className="teacher-detail-panel">
            <div className="teacher-detail-head">
              <div>
                ...
                <p>{selectedStudent.email || 'Sin email'} - {selectedStudent.role || 'student'} - {formatStatus(selectedStudent.status || 'active')}</p>
              </div>
          ...
          <Table
            ...
            render={(row) => (
              <tr key={row.id} className="teacher-click-row" onClick={() => setSelectedStudent(row)}>
                ...
                <td>{row.email || '-'}</td>
                <td><span className="teacher-status">{formatStatus(row.status || 'active')}</span></td>
