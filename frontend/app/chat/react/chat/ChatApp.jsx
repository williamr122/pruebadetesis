...
function getProfile() {
  try {
    const activeId = readStorage('yelia_active_student_id', '');
    const activeAlias = readStorage('yelia_active_student', '');
    const scopedKey = activeId || studentIdFromAlias(activeAlias);
    if (scopedKey) {
      const scoped = JSON.parse(readStorage(`yelia_profile_${scopedKey}`, '{}') || '{}');
      if (scoped?.alias) return scoped;
    }
    const global = JSON.parse(readStorage('yelia_profile', '{}') || '{}');
    if (global?.alias && (!activeAlias || global.alias === activeAlias)) return global;
    return activeAlias ? { alias: activeAlias } : {};
  } catch {
    return {};
  }
}
...
function saveProfile(profile) {
  ...
  const alias = String(next.alias || '').trim();
  if (alias) {
    const studentId = studentIdFromAlias(alias);
    writeStorage('yelia_active_student', alias);
    writeStorage('yelia_active_student_id', studentId);
    writeStorage(`yelia_profile_${studentId}`, JSON.stringify(next));
  }
}

function studentIdFromAlias(value = '') {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (/^STU-/i.test(raw)) return `STU-${raw.slice(4).trim().replace(/\s+/g, '-')}`;
  return `STU-${raw.replace(/[^A-Za-z0-9._ -]/g, '').trim().replace(/\s+/g, '-')}`;
}

function displayNameFromUser(value = '') {
  const raw = String(value || '').trim();
  if (!raw) return '';
  if (/^STU-/i.test(raw)) return raw.slice(4).replace(/[-_]+/g, ' ').trim();
  if (/^GUEST-/i.test(raw)) return `Invitado ${raw.slice(6).replace(/[-_]+/g, ' ').trim()}`;
  if (/^Anon-/i.test(raw)) return 'Invitado 1';
  return raw;
}
...
function readCurrentUnit() {
  if (typeof window === 'undefined') return 1;
  try {
    const rawRoute = localStorage.getItem('yelia_route_state');
    if (rawRoute) {
      const parsed = JSON.parse(rawRoute);
      if (parsed && parsed.currentUnit) {
        return Number(parsed.currentUnit);
      }
    }
  } catch {}
  try {
    const rawUnit = localStorage.getItem('yelia_route_unit');
    if (rawUnit) return Number(rawUnit);
  } catch {}
  return 1;
}
      ...
      {m.role !== 'user' && !m.welcome && (
        <div className="yelia-message-tools">
          <div className="yelia-message-tool-row">
            ...
            <button onClick={() => onQuick?.('Explicame mejor esta respuesta', m)} type="button">Explicame mejor</button>
            <button onClick={() => onQuick?.('Dame un ejemplo practico de esta respuesta', m)} type="button">Dame ejemplo</button>
            <button onClick={() => onQuick?.('Hazme un quiz corto sobre esta respuesta', m)} type="button">Hazme quiz</button>
          </div>
...
function RecommendationItem({ item, index, onSelect }) {
  ...
  const type = String(normalized.resource_type || normalized.type || '').toLowerCase();
  
  const iconMap = {
    pdf: 'bi-file-earmark-pdf',
    presentation: 'bi-easel',
    workshop: 'bi-pencil-square',
    lesson: 'bi-bookmark-check',
    exam: 'bi-clipboard-check',
    unit_content: 'bi-file-earmark-pdf',
  };
  const icon = iconMap[type] || (index % 3 === 0 ? 'bi-journal-code' : index % 3 === 1 ? 'bi-ui-checks' : 'bi-collection');
  if (normalized.url) {
    return (
...
export default function ChatApp() {
  ...
  const displayUser = profile?.alias?.trim() || (guestMode ? 'Invitado 1' : (displayNameFromUser(user) || 'Invitado 1'));
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      const activeAlias = String(localProfile.alias || readStorage('yelia_active_student', '') || '').trim();
      let effectiveGuest = localGuest || !activeAlias;
      let who = null;
      try {
        if (activeAlias && !localGuest) {
          await api.post('/api/auth/login', { student_code: activeAlias });
          writeStorage('yelia_active_student', activeAlias);
          writeStorage('yelia_active_student_id', studentIdFromAlias(activeAlias));
          writeStorage('yelia_guest_mode', '0');
          effectiveGuest = false;
        }
        who = await api.get('/api/auth/whoami');
        if (!alive) return;
        const isGuest = effectiveGuest || who?.mode === 'guest' || String(who?.usuario || '').startsWith('GUEST-') || String(who?.usuario || '').startsWith('Anon-');
        const visibleName = activeAlias || localProfile.alias || who?.nickname || (isGuest ? 'Invitado 1' : displayNameFromUser(who?.usuario)) || 'Invitado 1';
        setGuestMode(isGuest);
        setUser(visibleName);
      } catch {
        if (!alive) return;
        setGuestMode(effectiveGuest);
        setUser(effectiveGuest ? 'Invitado 1' : (activeAlias || localProfile.alias || 'Invitado 1'));
      }
      try {
        const progressData = await api.get('/api/progreso');
        if (alive && progressData?.progreso) {
          const prog = progressData.progreso;
          const isStudent = who?.mode === 'student' || String(who?.usuario || '').startsWith('STU-');
          const isGuest = effectiveGuest || who?.mode === 'guest' || String(who?.usuario || '').startsWith('GUEST-') || String(who?.usuario || '').startsWith('Anon-');
          if (isStudent && !isGuest && !prog.nivel_materia) {
  ...
  async function handleProfileSave(nextProfile) {
    ...
    try {
      if (alias) {
        ...
        writeStorage('yelia_active_student', alias);
        writeStorage('yelia_active_student_id', studentIdFromAlias(alias));
  ...
  async function continueAsGuest() {
    ...
    removeStorage('yelia_active_student_id');
    saveProfile({});
    setProfileState({});
    setConversationId(null);
    setRecommendations([]);
  ...
  async function send(text = input, contextMessage = null) {
    ...
    const originalResponse = contextMessage ? (contextMessage.text || plain(contextMessage.html) || '') : null;
    const currentUnit = readCurrentUnit();
    try {
      const data = await api.post('/api/chat', {
        message: question,
        perfil: guestMode ? { ...profile, alias: '' } : profile,
        profile: guestMode ? { ...profile, alias: '' } : profile,
        student_id: guestMode ? '' : studentIdFromAlias(profile?.alias || user),
        user_id: guestMode ? '' : studentIdFromAlias(profile?.alias || user),
        topic: nextTopic,
        conversation_id: conversationId,
        original_response: originalResponse,
        unidad_actual: currentUnit,
        attachment_ids: activeAttachmentId ? [activeAttachmentId] : [],
      });
      ...
      if (data.structured_grade) {
        const score = Number(data.structured_grade.score || 0);
        const total = Number(data.structured_grade.quiz_total || data.structured_grade.total || 0);
      ...
      setBackendAvatar({ version: 'avatar.v1', state: 'speaking', emotion: 'neutral', expression: 'supportive', gesture: 'explain', intensity: 0.5, speaking: true, mouth_shape: 'talk' });
      setMessages((items) => [
        ...items,
        {
          role: 'bot',
          text: 'Puedo ayudarte con ese tema. Intenta escribirlo con otras palabras o dime a qué unidad pertenece para orientarte mejor.',
          time: messageTime(new Date()),
        },
      ]);
    } finally {
  ...
  async function upload(file) {
    ...
    const activeStudentId = guestMode ? '' : studentIdFromAlias(profile?.alias || user);
    if (activeStudentId) {
      form.append('student_id', activeStudentId);
      form.append('user_id', activeStudentId);
    }
    
    try {
      const data = await api.upload('/api/attachments/upload', form);
      const newConvId = data.conversation_id || data.data?.conversation_id || conversationId;
      setConversationId(newConvId);
      const attId = data?.attachment?.id || data?.data?.attachment?.id;
      if (attId) {
      ...
      refreshHistory();
      if (newConvId) {
        await loadConversation({ id: newConvId });
        if (voiceEnabled) {
          speak(`He leído el archivo ${file.name}. Puedes pedirme que lo resuma, que resuelva una pregunta o que explique un ejercicio.`);
        }
      }
    } catch (error) {
      notify(error.message, 'error');
  ...
  function quickReply(prompt, messageObj = null) {
    send(prompt, messageObj);
  }
  ...
  return (
    <>
      ...
      <header className="yelia-header yelia-redesign-header">
        <div className="header-left"><img src="/static/img/logo-fii.png" alt="Facultad de Ingenieria Industrial" /></div>
        <div className="header-center">
          <h1>YELIA4AP</h1>
          <p className="subtitle">Asistente Educativo Interactivo para Programacion Avanzada</p>
          <small>Proyecto Academico de Titulacion</small>
        </div>
        <div className="header-right"><img src="/static/img/logo-ug.png" alt="Universidad de Guayaquil" /></div>
      </header>
      ...
      <main className="yelia-main yelia-redesign-main">
        <div className={`yelia-redesign-layout ${drawerOpen ? 'is-drawer-open' : ''} ${avatarVisible ? 'is-avatar-open' : 'is-avatar-hidden'}`}>
              ...
              <div className="yelia-drawer-scroll yelia-appearance-panel">
                <DrawerSection title="Tema visual" icon="bi-palette">
                  ...
                  {availableVoices && availableVoices.length > 0 && (
                    <div className="yelia-font-control mt-3">
                      <span><b>Selección de voz</b><small>Elige el sintetizador para el avatar.</small></span>
                      <select 
                        className="yelia-voice-select form-select form-select-sm mt-1" 
                        style={{ background: 'var(--yelia-bg-card, #232733)', color: 'var(--yelia-text-primary, #ffffff)', borderColor: 'var(--yelia-border-light, rgba(255,255,255,0.1))', padding: '0.4rem', borderRadius: '6px' }}
                        value={selectedVoiceName} 
                        onChange={(e) => setSelectedVoiceName(e.target.value)}
                      >
                        <option value="">(Predeterminada / Natural)</option>
          ...
          <section className="yelia-panel yelia-chat-panel yelia-redesign-chat">
              ...
              <div className="yelia-chat-header-grid d-flex align-items-center justify-content-between">
                <div className="yelia-title-block">
                  <div className="yelia-title-row">
                    ...
                    <button className="yelia-icon-button yelia-title-control" onClick={() => setVoiceEnabled(!voiceEnabled)} type="button" title="Voz de respuestas" aria-label="Activar o desactivar voz de respuestas">
                  ...
                  <div className="yelia-topic-bar mt-2">
                    <span className="yelia-topic-label">Tema actual:</span>
                    <span className="yelia-topic-value">{topic}</span>
                  </div>
                </div>
                <div className="yelia-chat-header-actions d-flex align-items-center gap-2">
                  ...
                  <div className="yelia-chat-search-box">
                    <i className="bi bi-search"></i>
                    <input value={chatQuery} onChange={(e) => { setChatQuery(e.target.value); setChatSearchIndex(0); }} placeholder="Buscar en chat..." />
                    {chatQuery && (
                  ...
                  <a className="yelia-student-control-link" href="/progreso" title="Ver mi progreso y mapa de calor">
                    <i className="bi bi-bar-chart-line"></i>
                    Mi progreso
                  </a>
                  <a className="yelia-student-control-link" href="/ruta" title="Ver ruta academica por unidades">
            ...
            <div className="yelia-input-bar">
              ...
              <div className="yelia-composer-side yelia-composer-right" aria-label="Voz y envio">
                ...
                {speaking ? (
                  <button className="yelia-send-button-modern yelia-stop-button" onClick={() => stop()} type="button" title="Detener lectura" aria-label="Detener lectura" style={{ background: '#dc3545', borderColor: '#dc3545' }}>
                    <i className="bi bi-stop-fill" style={{ color: '#ffffff' }}></i>
                  </button>
                ) : (
          ...
          {avatarVisible && <AvatarPanel state={effectiveAvatarState} />}
        </div>
      </main>
...
function Confetti() {
  ...
  useEffect(() => {
    const colors = [
      ...
      '#ff5722', '#00e676', '#ff007f', '#1de9b6', '#d500f9'
    ];
    const count = 120;
    const items = Array.from({ length: count }).map((_, i) => {
      ...
      return {
        ...
        delay: `${Math.random() * 1.5}s`, // delay up to 1.5s
        duration: `${Math.random() * 2 + 2}s`, // duration 2s to 4s
        size: `${size}px`,
        isCircle,
      ...
      {particles.map((p) => (
        <div
          ...
          style={{
            ...
            left: p.left,
            width: p.size,
            height: p.isCircle ? p.size : `${Math.random() * 6 + 10}px`,
            backgroundColor: p.color,
            borderRadius: p.isCircle ? '50%' : '2px',
            opacity: Math.random() * 0.4 + 0.6,
            transform: `rotate(${p.tilt})`,
            animation: `yelia-confetti-fall ${p.duration} linear ${p.delay} forwards`,
          }}
        />
      ))}
    </div>
