...
export default function AdminPanel() {
  ...
  async function newStudent() {
    ...
    const course = newStudentForm.course.trim();
    const tags = newStudentForm.tags.trim();
    if (!course) {
      notify('Ciclo / semestre requerido.', 'error');
      return;
    }
    if (!tags) {
      notify('Estado respecto a la materia requerido.', 'error');
      return;
    }
    try {
      ...
      const studentId = created?.student_id || created?.user_id || `STU-${alias.replace(/\s+/g, '-')}`;
      const profilePayload = {
        course,
        tags,
      };
      await api.patch(`/api/admin/students/${encodeURIComponent(studentId)}/profile`, profilePayload);
      
      window.localStorage.setItem('yelia_active_student', alias);
      window.localStorage.setItem('yelia_active_student_id', studentId);
      window.localStorage.setItem('yelia_guest_mode', '0');
      window.localStorage.setItem(`yelia_profile_${studentId}`, JSON.stringify({ alias, ciclo: course, estado: tags, nivel: 'Por diagnosticar' }));
      window.localStorage.setItem('yelia_profile', JSON.stringify({ alias, ciclo: course, estado: tags, nivel: 'Por diagnosticar' }));
  ...
  function studentLabel(row) {
    const raw = row?.display_name || row?.student_alias || row?.alias || row?.usuario || row?.username || row?.email || `Estudiante ${row?.id || ''}`;
    if (String(raw).startsWith('GUEST-')) return `Invitado ${String(raw).slice(6).replace(/[-_]+/g, ' ')}`;
    if (String(raw).startsWith('STU-')) return String(raw).slice(4).replace(/[-_]+/g, ' ');
  ...
  function formatStatus(status) {
    ...
    if (s === 'blocked' || s === 'inactive') return 'Desconectado';
        ...
        <div className="page">
          ...
          {view === 'students' && <section>
            ...
            {showStudentForm ? <div className="admin-create-student-card">
              ...
              <select className="form-select" value={newStudentForm.status} onChange={(e) => setNewStudentForm({ ...newStudentForm, status: e.target.value })}><option value="active">Activo</option><option value="blocked">Desconectado</option></select>
              
              <select className="form-select" value={newStudentForm.course} onChange={(e) => setNewStudentForm({ ...newStudentForm, course: e.target.value })}>
                <option value="">Seleccionar ciclo / semestre...</option>
              ...
              <select className="form-select" value={newStudentForm.tags} onChange={(e) => setNewStudentForm({ ...newStudentForm, tags: e.target.value })}>
                <option value="">Seleccionar estado respecto a la materia...</option>
                <option value="No la veo aun">No la veo aun</option>
                <option value="La estoy cursando">La estoy cursando</option>
                <option value="Aprendiendo">Aprendiendo</option>
                <option value="Repitiendo">Repitiendo</option>
                <option value="Ya la vi">Ya la vi</option>
                <option value="Ya la aprobe">Ya la aprobe</option>
                <option value="Repasando">Repasando</option>
                <option value="Preparando examen">Preparando examen</option>
              </select>
            ...
            {editingStudent ? <div className="admin-edit-card">
              ...
              <select className="form-select" value={editingStudent.status || 'active'} onChange={(e) => setEditingStudent({ ...editingStudent, status: e.target.value })}><option value="active">Activo</option><option value="blocked">Desconectado</option></select>
            ...
            {selectedStudent ? <div className="admin-detail-panel">
              <div className="admin-detail-head">
                <div><span className="admin-detail-tag">Detalle estudiante</span><h3>{studentLabel(selectedStudent)}</h3><p>{selectedStudent.email || 'Sin email'} - {selectedStudent.role || 'student'} - {formatStatus(selectedStudent.status || 'active')}</p></div>
          ...
          {view === 'accounts' && <section>
            ...
            {editingAccount ? <div className="admin-edit-card">
              ...
              <select className="form-select" value={editingAccount.status || 'active'} onChange={(e) => setEditingAccount({ ...editingAccount, status: e.target.value })}><option value="active">Activo</option><option value="blocked">Desconectado</option></select>
            ...
            <Table cols={['ID', 'Usuario', 'Email', 'Rol', 'Estado', 'Creado', 'Acciones']} rows={accounts} render={(r) => <tr key={r.id}><td>{r.id}</td><td>{r.username}</td><td>{r.email}</td><td>{r.role}</td><td>{formatStatus(r.status)}</td><td>{fmtDate(r.created_at)}</td><td className="text-end"><div className="admin-row-actions"><button className="btn btn-sm btn-outline-light" onClick={() => setEditingAccount({ ...r, password: '' })}>Editar</button><button className="btn btn-sm btn-outline-danger" onClick={() => delAccount(r.id)}>Eliminar</button></div></td></tr>} />
          </section>}
