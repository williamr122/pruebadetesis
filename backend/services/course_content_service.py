...
def final_question_bank(total: int = 10) -> List[Dict[str, Any]]:
    ...
    while len(questions) < total:
        ...
        extra_index += 1
    res = []
    for idx, item in enumerate(questions[:total]):
        clone = dict(item)
        clone["id"] = f"final-{idx+1:02d}"
        res.append(clone)
    return res
...
def unit_content(unit_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "workshop_questions": [
            _normalize_question(item, fallback_unit=safe_unit, kind="workshop")
            for item in unit.get("workshop_questions") or []
            if item.get("question") and len(item.get("options") or []) >= 2
        ],
