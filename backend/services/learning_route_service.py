...
def _default_route() -> Dict[str, Any]:
    return {
        "currentUnit": 1,
        "units": {
            "1": {"status": "active", "progress": 0, "quiz": None, "practice_started": False},
...
def _normalize_route(data: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    # Recalculate progress dynamically to ensure it is based strictly on real actions
    for key, unit in route["units"].items():
        progress = 0
        if unit.get("content_done"):
            progress = 20
            if unit.get("presentation_done"):
                progress = 40
                if unit.get("workshop_done"):
                    progress = 60
                    if unit.get("lesson_done"):
                        progress = 80
                        is_exam_done = unit.get("status") == "done" or (isinstance(unit.get("quiz"), dict) and unit.get("quiz", {}).get("passed"))
                        if is_exam_done:
                            progress = 100
                            unit["status"] = "done"
                        else:
    ...
    # Ensure sequential status unlocking
    # U1 is active by default.
    # If U1 is done, U2 is active.
    # If U2 is done, U3 is active.
    # If U3 is done, U4 is active.
    # Any unit that shouldn't be active/done yet is locked.
    ...
    # Set currentUnit to the first active unit
    first_active = None
    for i in range(1, 5):
        if route["units"][str(i)]["status"] == "active":
            first_active = i
            break
    if first_active is None:
        route["currentUnit"] = 4
    else:
        route["currentUnit"] = first_active
...
def grade_quiz(usuario: str, unit_id: int, answers: Dict[str, Any]) -> Dict[str, Any]:
    ...
    unit_state["progress"] = 100 if passed else max(int(unit_state.get("progress") or 0), 80)
    ...
    if passed and unit_id < 4:
        ...
        if next_state.get("status") == "locked":
            ...
            next_state["progress"] = max(int(next_state.get("progress") or 0), 0)
...
def update_unit_step(usuario: str, unit_id: int, step: str, completed: bool) -> Dict[str, Any]:
    route = get_route(usuario)
    key = str(max(1, min(4, int(unit_id or 1))))
    unit = route["units"][key]
    if unit.get("status") == "locked":
    ...
    unit[f"{step}_done"] = completed
    
    # Adjust general progress value
    current_progress = int(unit.get("progress") or 0)
    if step == "content" and completed:
        unit["progress"] = max(current_progress, 20)
    elif step == "presentation" and completed:
        unit["progress"] = max(current_progress, 40)
    elif step == "videos" and completed:
        pass
    elif step == "workshop" and completed:
        unit["progress"] = max(current_progress, 60)
        unit["practice_started"] = True
    elif step == "lesson" and completed:
        unit["progress"] = max(current_progress, 80)
