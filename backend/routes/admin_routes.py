...
def _clean_student_alias(value: str) -> str:
    raw = str(value or "").strip()
    cleaned = re.sub(r"[^A-Za-z0-9._ -]", "", raw)[:48].strip()
    return cleaned or "Estudiante"


def _student_user_id(alias: str) -> str:
    cleaned = _clean_student_alias(alias)
    if cleaned.upper().startswith("STU-"):
        return "STU-" + cleaned[4:].strip()
    return "STU-" + "-".join(cleaned.split())


def _student_display_name(student_id: str) -> str:
    value = str(student_id or "").strip()
    if value.upper().startswith("STU-"):
        return value[4:].replace("-", " ").strip() or value
    if value.upper().startswith("GUEST-"):
        return "Invitado " + value[6:].replace("-", " ").strip()
    return value


def _attach_student_display(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    for row in rows:
        student_id = row.get("alias") or row.get("usuario") or ""
        display = None
        try:
            profile = student_profile_repo.get_profile(str(student_id))
            display = (profile or {}).get("nickname")
        except Exception:
            display = None
        row["student_id"] = student_id
        row["user_id"] = student_id
        row["display_name"] = display or _student_display_name(str(student_id))
        row["student_alias"] = row["display_name"]
    return rows
...
def api_students_list():
    ...
    if request.method == "POST":
        ...
        alias_visible = _clean_student_alias(body.get("alias") or "")
        if not alias_visible:
            return _err("Alias requerido.", 400)
        alias = _student_user_id(alias_visible)
        ...
        try:
            ...
            # Inicializar perfil por defecto completo
            student_profile_repo.save_profile(alias, student_profile_repo._default_profile(alias))
            student_profile_repo.set_nickname(alias, alias_visible)
            
            # Inicializar registro de progreso en progreso
            from backend.services.progreso_service import actualizar_perfil_usuario
            actualizar_perfil_usuario(alias)
            
            # Inicializar ruta de aprendizaje en learning_routes
            from backend.services.learning_route_service import get_route
            get_route(alias)
        ...
        _audit("student.create", target=alias, meta={"email": email, "display_name": alias_visible})
        return _ok({"id": new_id, "alias": alias_visible, "student_id": alias, "user_id": alias}, 201)
    ...
    with db_session() as conn:
        ...
        rows = _attach_student_display([dict(r) for r in cur.fetchall()])
...
def api_students_delete(user_id: int):
    ...
    try:
        with db_session(write=True) as conn:
            ...
            # Create deleted_students table if it does not exist
            cur.execute(
                """
            ...
            for a in aliases_to_delete:
                cur.execute("SELECT 1 FROM deleted_students WHERE alias = ?;", (a,))
                if not cur.fetchone():
                    cur.execute("INSERT INTO deleted_students (alias) VALUES (?);", (a,))
...
def api_student_activity(alias: str):
    ...
    display_alias = _clean_student_alias(alias)
    stu_usuario = _student_user_id(display_alias)
    ...
    with db_session() as conn:
        ...
        cur.execute(
            ...
            SELECT id,
                   ...
                   created_at
            FROM interacciones
            WHERE usuario_id IN (SELECT id FROM usuarios WHERE LOWER(alias) IN (LOWER(?), LOWER(?)))
...
def api_student_profile_get(alias: str):
    ...
    student_id = _student_user_id(alias)
    profile = student_profile_repo.get_profile(student_id)
...
def api_student_profile_patch(alias: str):
    ...
    student_id = _student_user_id(alias)
    current = student_profile_repo.get_profile(student_id)
    ...
    if not merged.get("nickname"):
        merged["nickname"] = _student_display_name(student_id)
        
    # Delegar almacenamiento de ciclo, estado y nivel usando actualizar_perfil_usuario
    from backend.services.progreso_service import actualizar_perfil_usuario
    course = updates.get("course")
    tags = updates.get("tags")
    lvl = updates.get("level_current")
    
    ciclo_val = str(course).strip() if course else None
    estado_val = ", ".join(tags) if isinstance(tags, list) else (str(tags).strip() if tags else None)
    
    actualizar_perfil_usuario(student_id, ciclo_academico=ciclo_val, estado_materia=estado_val, nivel_materia=lvl)
    
    _audit("student.profile.update", target=student_id)
    return _ok({"message": "Perfil actualizado.", "profile": student_profile_repo.get_profile(student_id)})
