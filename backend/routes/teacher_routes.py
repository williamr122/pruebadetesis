...
def _attach_student_display(rows: list[dict]) -> list[dict]:
    for row in rows:
        student_id = row.get("alias") or row.get("usuario") or ""
        try:
            profile = get_profile(str(student_id)) or {}
            display = profile.get("nickname")
        except Exception:
            display = None
...
def api_teacher_dashboard():
    ...
    with db_session() as conn:
        ...
        cur.execute(
            ...
            WHERE LOWER(COALESCE(u.status, 'active')) NOT IN ('blocked', 'inactive');
            """
        )
        active_students = int(cur.fetchone()[0] or 0)
