...
def index():
    ...
    - La entrada principal del sistema es el **Launcher** (hub), porque concentra:
      chat, paneles, diagnóstico y accesos rápidos.
    - El chat sigue disponible como acceso directo en "/chat".

    Configuración:
    - HOME_PAGE=launcher  -> redirige a /launcher (default)
    - HOME_PAGE=chat      -> redirige a /chat
    """
    home = (os.getenv("HOME_PAGE") or "launcher").strip().lower()
    if home == "chat":
