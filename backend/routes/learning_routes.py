...
@chat_bp.route("/api/learning-route/unit/<int:unit_id>/step", methods=["POST"])
@limiter.exempt
def api_learning_unit_step(unit_id: int):
    usuario = obtener_usuario_actual()
    data = request.get_json(silent=True) or {}
    step = data.get("step")
    completed = bool(data.get("completed", True))
    if not step or step not in {"content", "presentation", "videos", "workshop", "lesson"}:
        return _err("Paso invalido.", 400)
    try:
        from backend.services.learning_route_service import update_unit_step
        return _ok(update_unit_step(usuario, unit_id, step, completed))
    except Exception as exc:
        logger.error("Error actualizando paso de unidad", usuario=usuario, unit_id=unit_id, step=step, error=str(exc))
        return _err("No se pudo actualizar el paso de la unidad.", 500)
