...
def api_auth_login():
    ...
    usuario = code if code.upper().startswith("STU-") else f"STU-{code}"
    ...
    try:
        ...
        logger.warning("No se pudo actualizar estado a active en login", usuario=usuario, error=str(e))
...
def api_auth_logout():
    ...
    if usuario and usuario.startswith("STU-"):
        try:
            with db_session(write=True) as conn:
                cur = conn.cursor()
                cur.execute("UPDATE usuarios SET status = 'blocked', updated_at = CURRENT_TIMESTAMP WHERE alias = ?;", (usuario,))
                conn.commit()
        except Exception as e:
            logger.warning("No se pudo actualizar estado a blocked en logout", usuario=usuario, error=str(e))

    session.pop("usuario", None)
    session.pop("auth_mode", None)
    session.permanent = True
    usuario_guest = obtener_usuario_actual()
    logger.info("Auth logout", usuario=usuario_guest)
    return _ok({"usuario": usuario_guest})
