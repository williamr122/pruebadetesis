...
def api_recommendations():
    ...
    """
    data = request.get_json(silent=True) or {}
    requested_user = str(data.get("student_id") or data.get("user_id") or "").strip()
    if requested_user and not requested_user.startswith(("GUEST-", "Anon-")):
        if not requested_user.upper().startswith("STU-"):
            safe = "".join(ch for ch in requested_user if ch.isalnum() or ch in ("-", "_", ".", " "))
            requested_user = "STU-" + "-".join(safe.split())
        else:
            requested_user = "STU-" + requested_user[4:].strip()
        if requested_user != "STU-":
            session["usuario"] = requested_user
            session["auth_mode"] = "student"
            session.permanent = True
            try:
                get_or_create_usuario(requested_user)
            except Exception:
                pass
...
def api_chat():
    ...
    if not attachment_ids and conv_id:
        try:
            with db_session(write=False) as conn:
                cur = conn.cursor()
                cur.execute(
                    "SELECT id FROM attachments WHERE conv_id = ? AND usuario = ? ORDER BY id DESC LIMIT 1;",
                    (int(conv_id), usuario),
                )
                row = cur.fetchone()
                if row:
        ...
        attachment_ids = attachment_ids[-1:] if isinstance(attachment_ids, list) and attachment_ids else []
    ...
    if isinstance(attachment_ids, list) and attachment_ids:
        try:
            for raw_id in attachment_ids:
                ...
                # Resolver en PRIVATE_UPLOAD_DIR (y fallback a static/uploads si hay adjuntos antiguos)
                ...
                if extracted_text and extracted_text.strip():
                    attachment_context_parts.append(
                        f"[CONTENIDO DEL ARCHIVO ADJUNTADO: {original_name}]\n{extracted_text.strip()}\n"
                    )
                else:
                    ...
                    if r.ok and (r.text or "").strip():
                        ...
                        try:
                            save_attachment_analysis(att_id, extracted_text=text_val, extracted_meta=r.meta or {})
                        except Exception:
                            pass
                    else:
                        error_msg = r.error or "El archivo no contiene texto legible."
                        return jsonify({
                            "ok": True,
                            "respuesta": f"No he podido procesar el archivo '{original_name}' debido a un problema: {error_msg}",
                            "reply": f"No he podido procesar el archivo '{original_name}' debido a un problema: {error_msg}",
                            "tema": "Adjunto",
                            "modo": "error",
    ...
    try:
        ...
        if active_quiz and (detected_intent == "evaluacion_respuesta" or quiz_answers):
            ...
            unidad_actual = data.get("unidad_actual")
        ...
        if not texto_respuesta:
            texto_respuesta = "Puedo ayudarte con ese tema. Intenta escribirlo con otras palabras o dime a qué unidad pertenece para orientarte mejor."
            modo = "local"
            modo_interaccion = "normal"
            intencion = "otro"
        ...
        logger.exception("Error critico en api_chat, activando fallback local", error=str(e), usuario=usuario)
        
        pregunta_safe = "Consulta"
        try:
            pregunta_safe = mensaje or raw_user_message or data.get("message", "")
        except Exception:
            pass
        ...
        try:
            if conv_id:
                with db_session(write=True) as conn:
                    ...
                    cur.execute(
                        "INSERT INTO messages (conv_id, usuario, remitente, contenido, tema) VALUES (?, ?, 'user', ?, ?);",
                        (conv_id, usuario, pregunta_safe, tema),
                    )
                    cur.execute(
...
def upload_attachment():
    ...
    requested_user = str(request.form.get("student_id") or request.form.get("user_id") or "").strip()
    if requested_user and not requested_user.startswith(("GUEST-", "Anon-")):
        if not requested_user.upper().startswith("STU-"):
            safe = "".join(ch for ch in requested_user if ch.isalnum() or ch in ("-", "_", ".", " "))
            requested_user = "STU-" + "-".join(safe.split())
    ...
    r = extract_text_from_file(out_path)
    if not r.ok or not (r.text or "").strip():
        try:
            ...
            pass
        error_msg = r.error or "El archivo no contiene texto legible."
        return _err(f"No he podido procesar el archivo '{original_name}': {error_msg}", 400)
    ...
    if not conversation_id:
        try:
            conversation_id = crear_conversacion(usuario, titulo=f"Chat con {original_name}")
        except Exception:
            pass
    ...
    try:
        ...
        # Save analysis immediately in DB
        try:
            save_attachment_analysis(att_id, extracted_text=extracted_text, extracted_meta=r.meta or {})
        except Exception as e:
            logger.error("No se pudo guardar analisis del adjunto al subir", error=str(e), att_id=att_id)

        # Insert chat message for visual tracking
        if conversation_id:
            try:
                with db_session(write=True) as conn:
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT INTO messages (conv_id, usuario, remitente, contenido, tema) VALUES (?, ?, 'user', ?, ?);",
                        (conversation_id, usuario, f"📎 Archivo adjunto: {original_name}", "Adjunto"),
                    )
                    conn.commit()
            except Exception as e:
                logger.error("No se pudo insertar mensaje de adjunto", error=str(e))

            # Insert auto-reply from YELIA
            try:
                auto_reply_text = (
                    f"He leído el archivo '{original_name}'. "
                    "Puedes pedirme que lo resuma, que resuelva una pregunta o que explique un ejercicio."
                )
                with db_session(write=True) as conn:
                    cur = conn.cursor()
                    cur.execute(
                        "INSERT INTO messages (conv_id, usuario, remitente, contenido, tema, proveedor, response_ms) "
                        "VALUES (?, ?, 'bot', ?, 'Adjunto', 'local', 0);",
                        (conversation_id, usuario, auto_reply_text),
                    )
                    conn.commit()
            except Exception as e:
                logger.error("No se pudo insertar auto-reply de adjunto", error=str(e))
