...
def _diagnostic_done(usuario: str) -> bool:
    ...
    with db_session() as conn:
        cur = conn.cursor()
        cur.execute("SELECT id FROM diagnostic_attempts WHERE LOWER(usuario) = LOWER(?) ORDER BY id DESC LIMIT 1;", (usuario,))
        if cur.fetchone():
            return True
...
def _verify_student_password(usuario: str, password: str, create: bool = False):
    with db_session(write=True) as conn:
        cur = conn.cursor()
        cur.execute("SELECT alias, password_hash, status FROM usuarios WHERE LOWER(alias) = LOWER(?);", (usuario,))
        row = cur.fetchone()
        if row:
            usuario = row["alias"]
        if row and (row["status"] or "active") != "active":
            return False, "Cuenta de estudiante bloqueada.", usuario
        existing_hash = row["password_hash"] if row else None
        if existing_hash:
            if not password or not check_password_hash(existing_hash, password):
                return False, "Codigo o clave de estudiante incorrectos.", usuario
            return True, "", usuario
        if not create:
            return True, "", usuario
        password_error = validate_password(password or "", min_len=6, label="La clave del estudiante")
        if password_error:
            return False, password_error, usuario
        cur.execute("SELECT id FROM usuarios WHERE alias = ?;", (usuario,))
        if not cur.fetchone():
            cur.execute("INSERT INTO usuarios (alias) VALUES (?);", (usuario,))
        cur.execute(
...
def api_diagnostic_status():
    ...
    try:
        if mode != "guest":
            ok, message, corrected_usuario = _verify_student_password(usuario, password, create=False)
            if not ok:
                return _err(message, 401)
            usuario = corrected_usuario

        # Establecer sesión de inmediato si la autenticación fue exitosa
        ...
        if mode != "guest":
            try:
                with db_session(write=True) as conn:
                    cur = conn.cursor()
                    cur.execute("UPDATE usuarios SET status = 'active', updated_at = CURRENT_TIMESTAMP WHERE alias = ?;", (usuario,))
                    conn.commit()
            except Exception as e:
                logger.warning("No se pudo actualizar estado a active en api_diagnostic_status", usuario=usuario, error=str(e))

        done = _diagnostic_done(usuario)

        # Cargar ciclo y estado desde el perfil académico en student_profiles (única fuente de verdad)
        ciclo = None
        estado = None
        nivel = "Por diagnosticar"
        try:
            from backend.repositories.student_profile_repo import get_profile
            prof = get_profile(usuario)
            ciclo = prof.get("ciclo") or prof.get("course")
            estado = prof.get("estado")
            if not estado and prof.get("tags"):
                t = prof.get("tags")
                estado = ", ".join(t) if isinstance(t, list) else str(t)
            nivel = prof.get("level_current") or "Por diagnosticar"
        except Exception:
            pass

        return _ok({
            "usuario": usuario,
            "alias": usuario[4:] if usuario.startswith("STU-") else (usuario[6:] if usuario.startswith("GUEST-") else alias),
            "mode": mode,
            "diagnostic_completed": done,
            "ciclo": ciclo,
            "estado": estado,
            "nivel": nivel
        })
...
def api_diagnostic_submit():
    ...
    usuario = _guest_user(alias) if mode == "guest" else _student_user(alias)
    if mode != "guest":
        ok, message, corrected_usuario = _verify_student_password(usuario, password, create=True)
        if not ok:
    ...
    if mode != "guest":
        try:
            ...
            logger.warning("No se pudo actualizar estado a active en api_diagnostic_submit", usuario=usuario, error=str(e))
    ...
    try:
        _ensure_attempts_table()
        # Guardar ciclo y estado en student_profiles, y nivel obtenido en progreso
