...
# Apply Javascript cleanup factor
cleaned_offsets = np.copy(boca_offsets)
for i in range(len(base_positions)):
    ...
    if absY >= 0.05:
        ...
        factor = (0.05 - absY) / 0.03
    cleaned_offsets[i] *= factor

# Find vertices with large cleaned offset
norms = np.linalg.norm(cleaned_offsets, axis=1)
large_indices = np.where(norms > 0.01)[0]
print(f"Number of vertices with clean offset > 0.01: {len(large_indices)}")

# Group them by Z (vertical) to see what moves up or down
# Let's sort by norm of cleaned offset descending
sorted_indices = large_indices[np.argsort(norms[large_indices])[::-1]]
for idx in sorted_indices[:20]:
    print(f"Index {idx}: Base={base_positions[idx]}, CleanedOffset={cleaned_offsets[idx]}, Norm={norms[idx]:.4f}")
