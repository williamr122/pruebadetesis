...
for i in range(len(base_positions)):
    ...
    if x > 0.45 and abs(y) < 0.05:
        beak_vertices.append((i, x, y, z))

print(f"Found {len(beak_vertices)} beak/front vertices with X > 0.45 and abs(Y) < 0.05.")

# Print their offsets for boca and ojo
for idx, x, y, z in beak_vertices[:30]:
    b_off = boca_offsets[idx]
    o_off = ojo_offsets[idx]
    print(f"Index {idx:5d}: Pos=[{x:5.3f}, {y:5.3f}, {z:5.3f}], BocaOff=[{b_off[0]:5.3f}, {b_off[1]:5.3f}, {b_off[2]:5.3f}], OjoOff=[{o_off[0]:5.3f}, {o_off[1]:5.3f}, {o_off[2]:5.3f}]")
